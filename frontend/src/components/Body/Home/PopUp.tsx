...
const PopUpCard = ({
    ...
    const headerTheme = 'bg-blue-900/30 border-blue-700'

    const textTheme = 'text-slate-400'

    const labelTheme = 'text-slate-500'

    const buttonPrimary = 'bg-blue-600 hover:bg-blue-700 text-white'

    const buttonSecondary = 'bg-slate-700 hover:bg-slate-600 text-slate-200'

    return (
        <div className={`rounded-xl border ${cardTheme} overflow-hidden`}>
            {/* Header with icon */}
            <div
                className={`px-4 py-3 border-b flex items-center gap-2 ${headerTheme}`}
            >
                ...
                <div className="space-y-2 text-sm">
                    {detectedJob?.location && (
                        <div className="flex items-start gap-2">
                            <span
                                className={`text-xs font-medium ${labelTheme} w-20 shrink-0`}
                            >
                                Location:
                            </span>
